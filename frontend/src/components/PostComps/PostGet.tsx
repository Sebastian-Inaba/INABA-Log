...
export default function PublicPostList() {
    ...
    const currentItems = filteredPosts.slice(page * itemsPerPage, (page + 1) * itemsPerPage);
        ...
        return (
            <div className="h-full min-h-0 flex items-center justify-center">
                <div className="bg-red-900 border border-red-700 text-red-200 p-4 rounded-lg max-w-lg w-full">
                    ...
                    <button onClick={() => void fetchPosts()} className="px-4 py-2 bg-red-700 text-white rounded-lg hover:bg-red-600 mt-2">
                        Try Again
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="flex flex-col h-full min-h-0 w-full">
            ...
            <div className="flex items-center justify-end p-2">
                <div className="text-sm text-purple-300">Total: {filteredPosts.length} posts</div>
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex flex-col min-w-0">
                                            ...
                                            <h3 className="text-xl font-semibold text-white mb-1 truncate">{post.title}</h3>
                                            <p className="text-neutral-400 text-sm mb-2 line-clamp-2">{post.description}</p>
                                            ...
                                            <div className="flex items-center gap-4 text-xs text-neutral-500 truncate">
                                                ...
                                                <span>Updated: {post.updatedAt ? formatDate(post.updatedAt) : '—'}</span>
                                                {post.author && <span className="truncate">By: {post.author}</span>}
                                            </div>
                                        </div>
                                    </div>
                                );
