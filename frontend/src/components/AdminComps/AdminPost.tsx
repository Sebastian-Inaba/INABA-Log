...
// todo:
// remove and change api, helpers and types
// add robust responsiveness for mobile and tablet
// split up this component in smaller components
// oh and add auth hook api via new api utils
...
export default function CreateNewModal() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStatus({ error: err instanceof Error ? err.message : 'Unknown error', success: null });
    } finally {
      setLoading(false);
      setShowConfirm(false);
    }
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Create new content">
          ...
          <div className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-neutral-900 text-white rounded-xl border border-purple-700 shadow-2xl p-6 z-60">
            <div className="flex items-start justify-between gap-4 mb-6">
              ...
              <button onClick={() => setOpen(false)} className="text-xl font-bold px-2 py-1 hover:bg-purple-900 rounded-lg" aria-label="Close">×</button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-5">
                ...
                {type === 'research' && (
                  <div>
                    <label className="block text-sm font-semibold text-purple-300 mb-1">References (comma, semicolon, or newline separated)</label>
                    <textarea
                ...
                <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-purple-800">
                  <button onClick={() => { setOpen(false); }} className="px-4 py-2 bg-neutral-700 text-white rounded-lg hover:bg-neutral-600">
                    Cancel
                  </button>
                  <button
  ...
  <div className="border-l border-purple-800 pl-6">
    ...
    <div className="overflow-auto max-h-[70vh] p-5 bg-neutral-800 rounded-lg border border-purple-700">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {tags.split(',').filter(tag => tag.trim()).map((tag: string, index: number) => (
          <span key={index} className="px-3 py-1 bg-neutral-700 text-neutral-300 rounded-full text-sm">{tag.trim()}</span>
        ))}
      </div>
  ...
  <div className="fixed inset-0 z-70 flex items-center justify-center">
    ...
    <div className="relative z-80 bg-neutral-800 border border-purple-700 p-6 rounded-lg w-full max-w-md">
      ...
      <p className="text-neutral-300 mb-4">
        Are you sure you want to create this {type}? This action cannot be undone.
      </p>
      <div className="flex justify-end gap-3">
        <button onClick={onCancel} className="px-4 py-2 bg-neutral-700 text-white rounded-lg hover:bg-neutral-600">
          Cancel
        </button>
        <button onClick={onConfirm} disabled={loading || disabled} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50">
